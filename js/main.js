...
if (rsvpName) {
  rsvpName.value = guest && guest !== "bạn" ? guest : "";
}
...
rsvpForm?.addEventListener("submit", async (event) => {
  ...
  rsvpSubmit.textContent = "Đang lưu...";
  ...
  try {
    const response = await fetch("/api/rsvp", {
      ...
      body: JSON.stringify({
        name,
        guestSlug: slug || null,
        inviteName: guest || null,
        pageUrl: window.location.href
      })
    });

    const result = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(result.error || "Không lưu được xác nhận.");

    rsvpStatus.textContent = "Đã xác nhận rồi nha, cảm ơn bạn nhiều 💙";
    ...
    rsvpSubmit.textContent = "Xác nhận tham gia";
  }
});
